...
export function OrderSuccess({ orderNumber }: { orderNumber: string }) {
  ...
  useEffect(() => { const orders = JSON.parse(localStorage.getItem("burmese-house-orders-v1") || "[]") as StoredOrder[]; setOrder(orders.find((item) => item.orderNumber === orderNumber) ?? null); }, [orderNumber]);
