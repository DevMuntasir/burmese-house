...
export function ProductDetail({ product }: { product: Product }) {
  ...
  const images = useMemo(() => {
    const availableImages = [selectedVariant?.image, ...(product.images ?? [])].filter(
      (value, index, array): value is string => Boolean(value) && array.indexOf(value) === index,
    );
    return availableImages.length ? availableImages : ["/images/hero-chutney.png"];
  ...
  return <div className="container-shell py-8 sm:py-12"><div className="grid gap-9 lg:grid-cols-[1.06fr_.94fr] lg:gap-14">
    ...
    <div><p className="eyebrow">{product.category?.name ?? "Uncategorized"}</p><h1 className="display-title mt-3 text-4xl leading-tight sm:text-5xl">{product.title}</h1><div className="mt-4 flex flex-wrap items-center gap-3"><span className="text-2xl font-extrabold text-[#6f2742]">{formatPrice(price)}</span>{product.salePrice && <span className="text-base text-stone-400 line-through">{formatPrice(product.regularPrice)}</span>}{discount > 0 && <span className="bg-[#f2e5e9] px-2 py-1 text-xs font-bold text-[#6f2742]">Save {discount}%</span>}</div><div className="mt-3 flex items-center gap-4 text-xs text-stone-500"><span className="flex items-center gap-1 font-semibold text-stone-700"><Star size={13} fill="#ffad1f" strokeWidth={0} className="text-[#ffad1f]" /> {(product.rating ?? 4.8).toFixed(1)}</span><span>SKU: {selectedVariant?.sku ?? product.sku}</span>{product.soldCount ? <span>{product.soldCount}+ sold</span> : null}</div><p className="mt-6 border-t border-stone-200 pt-6 text-sm leading-7 text-stone-600">{product.shortDescription}</p>
      ...
      <div className="mt-7 space-y-3 border-y border-stone-200 py-5 text-sm"><p className="flex items-center gap-3"><Truck size={18} className="text-[#6f2742]" /><span><strong>Delivery:</strong> 1–2 days local express, 3–5 days nationwide</span></p><p className="flex items-center gap-3"><Check size={18} className="text-[#6f2742]" /><span><strong>Easy returns:</strong> Check before accepting delivery</span></p></div>
    </div>
  </div></div>;
}
