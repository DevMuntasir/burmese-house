...
export function ProductDetail({ product }: { product: Product }) {
  ...
  return <div className="container-shell py-8 sm:py-12"><div className="grid gap-9 lg:grid-cols-[1.06fr_.94fr] lg:gap-14">
    ...
    <div><p className="eyebrow">{product.category?.name ?? "Uncategorized"}</p><h1 className="display-title mt-3 text-4xl leading-tight sm:text-5xl">{product.title}</h1><div className="mt-4 flex flex-wrap items-center gap-3"><span className="text-2xl font-extrabold text-[#6f2742]">{formatPrice(price)}</span>{product.salePrice && <span className="text-base text-stone-400 line-through">{formatPrice(product.regularPrice)}</span>}{discount > 0 && <span className="bg-[#f2e5e9] px-2 py-1 text-xs font-bold text-[#6f2742]">Save {discount}%</span>}</div><div className="mt-3 flex items-center gap-4 text-xs text-stone-500"><span>SKU: {selectedVariant?.sku ?? product.sku}</span>{product.soldCount ? <span>{product.soldCount}+ sold</span> : null}</div><p className="mt-6 border-t border-stone-200 pt-6 text-sm leading-7 text-stone-600">{product.shortDescription}</p>
