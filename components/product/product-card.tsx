  ...
  return (
    <article className="group min-w-0 overflow-hidden rounded-[16px] border border-[#eeeeee] bg-white p-2 shadow-[0_3px_18px_rgba(20,20,20,.035)] sm:rounded-[20px] sm:p-2.5">
      ...
      <div className="px-1 pb-1 pt-2.5 sm:px-1.5 sm:pb-2 sm:pt-3">
        <div className="flex items-center justify-between gap-2">{product.category?.slug ? <Link href={`/category/${product.category.slug}`} className="truncate text-[9px] font-medium text-[#999] sm:text-[11px]">{categoryName}</Link> : <span className="truncate text-[9px] font-medium text-[#999] sm:text-[11px]">{categoryName}</span>}<span className="flex shrink-0 items-center gap-1 text-[9px] text-[#777] sm:text-[11px]"><Star size={11} fill="#ffad1f" strokeWidth={0} /> {(product.rating ?? 4.8).toFixed(1)}</span></div>
